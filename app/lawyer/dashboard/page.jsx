...
export default function LawyerDashboard() {
  ...
  useEffect(() => {
    if (!user?._id) return;
    fetch(`/api/bookings/byLawyer?lawyerId=${user._id}`)
      .then((res) => res.json())
      .then(setBookings);

    fetch(`/api/appointments/byLawyer?lawyerId=${user._id}`)
      .then((res) => res.json())
      .then(setAppointments);
  }, [user]);

  const stats = [
    { title: "Active Cases", value: bookings.length, icon: FileText, color: "text-blue-600" },
